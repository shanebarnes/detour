...
func (s *ShortcutAzureBlob) isMilestoneRequest(role Role, buffer []byte) (bool, int, error) {
	...
	if s.ctxt[role].contentLength == 0 {
		...
		if request, reqSize := GetHttpRequest(&payload); request != nil {
			...
			//logger.Println("\n\n", request, "\n\n")
			//logger.Println(s.ctxt[role].tag, "Request Method:", request.Method, "Size:", request.ContentLength)
			...
			//logger.Println(s.ctxt[role].tag, "ContentLength:", request.ContentLength, "proxy:", proxy)
			if role == Client && request.Method == "PUT" && (proxy || (s.impl.block && s.ctxt[role].dropData)) {
				...
				if !s.ctxt[role].dropData && s.impl.block && s.impl.exits >= 0 && s.ctxt[role].milestoneCount >= s.impl.exits /*&& s.ctxt[role].milestoneCount > 1*/ {
					//logger.Println(s.ctxt[role].tag, "Dropping data")
...
func (s *ShortcutAzureBlob) isMilestoneResponse(role Role, buffer []byte) (bool, int, error) {
	...
	if s.ctxt[role].contentLength == 0 {
		...
		if r, _ := GetHttpResponse(&payload); r != nil {
			...
			//logger.Println("\n\n", r, "\n\n")
			//logger.Println(s.ctxt[role].tag, "Response Status:", r.Status)
			...
			if role == Server && r.StatusCode == http.StatusCreated {
				if s.impl.exits >= 0 && s.ctxt[Client].milestoneCount >= s.impl.exits && atomic.LoadInt32(&s.requestCount) > atomic.LoadInt32(&s.responseCount) {
					...
					dropResponse = true
					takenBytes = len(buffer)
					//logger.Println(s.ctxt[role].tag, "Dropping response to client")
				}
			}
		}
	}
...
func (s *ShortcutAzureBlob) cachePushBack(role Role, blockLength int64, blockMd5 string) {
	...
	//logger.Println(s.ctxt[role].tag, "Increased cache size to", len(s.ctxt[role].blockCache))
}
...
func (s *ShortcutAzureBlob) cachePopFront(role Role) {
	...
	//logger.Println(s.ctxt[role].tag, "Decreased cache size to", len(s.ctxt[role].blockCache), "( Removed block of size", blockSize, ")")
}
...
func (s *ShortcutAzureBlob) blockPushBack(role Role, cacheId int, chunk []byte) {
	if cacheId >= 0 && cacheId < len(s.ctxt[role].blockCache) {
		...
		//logger.Println(s.ctxt[role].tag, "Block", cacheId, "size is now:", len(s.ctxt[role].blockCache[cacheId]))
	} else {
		logger.PrintlnError(s.ctxt[role].tag, "Invalid cache index", cacheId)
	}
}
...
func (s *ShortcutAzureBlob) updateContentLeft(role Role, contentLength int64) { // Update "distance" left to travel
	if contentLength > s.ctxt[role].contentLength {
		logger.PrintlnError(s.ctxt[role].tag, "content length mismatch (", contentLength, ">", s.ctxt[role].contentLength, ")")
		s.ctxt[role].contentLength = 0
	} else {
...
func (s *ShortcutAzureBlob) sendResponse(role Role, blockId int) {
	...
	if s.impl.exits >= 0 && s.ctxt[role].milestoneCount >= s.impl.exits {
		//logger.Println(s.ctxt[role].tag, " Sending fast response to client for block of size ", s.ctxt[role].blockCache[blockId].contentLength)
		//logger.Println(rsp, "\n\n")
